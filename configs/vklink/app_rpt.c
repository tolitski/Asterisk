 ...
 *
 * Forked from Jim Dixon's Allstar Repository to suit the VK Repeater Network
 *
 * Originally Ver 0.325
 *
 *
 * Re-included is IRLP Channel driver - Only to be used by existing users.
 *
 * Also included is modifications to DTMF and how it is sent
 * AlsaRadio drivers are included as well as a new Rpi Driver exclusivley to the Raspi
 *
...
#define	FUNCCHAR '*'
#define	ENDCHAR '#'
#define	EXTNODEFILE "/root/rpt_extnodes"
#define	NODENAMES "rpt/nodenames"
#define CONN_PGM "/usr/bin/wget"
#define DISC_PGM "/usr/bin/wget"
...
#define MAXCONNECTTIME_IRLP 2500
...
#define STATPOST_URL "http://status.nevarc.net.au/get_post.php"
...
enum {LINKMODE_OFF,LINKMODE_ON,LINKMODE_FOLLOW,LINKMODE_DEMAND,
	LINKMODE_GUI,LINKMODE_PHONE,LINKMODE_ECHOLINK,LINKMODE_TLB,LINKMODE_IRLP};
...
int	i;
	...
	i = strlen(them) + strlen(myrpt->p.discpgm) + 130;
	a = ast_malloc(i);
	if (!a) 
	{
		ast_log(LOG_NOTICE,"Unable to alloc");
	...
	memset(a,0,i);
	sprintf(a,"%s --delete-after \"http://status.nevarc.net.au/get_conns.php?doingwhat=disc&node=%s&node2=%s\" &",myrpt->p.discpgm,myrpt->name,them);
	ast_safe_system(a);
	free(a);
	return;
}
...
int	i;
	...
	i = strlen(them) + strlen(myrpt->p.connpgm) +  + 130;
	a = ast_malloc(i);
	if (!a) 
	{
		ast_log(LOG_NOTICE,"Unable to alloc");
	...
	memset(a,0,i);
	sprintf(a,"%s --delete-after \"http://status.nevarc.net.au/get_conns.php?doingwhat=conn&node=%s&node2=%s\" &",myrpt->p.connpgm,myrpt->name,them);
	ast_safe_system(a);
	ast_free(a);
	return;
}
...
static char *cs_keywords[] = {"rptena","rptdis","apena","apdis","lnkena","lnkdis","totena","totdis","skena","skdis",
	...
	val = (char *) ast_variable_retrieve(cfg,this,"statpost_local");
	if (!val) rpt_vars[n].p.statpost_url = STATPOST_URL;
		else rpt_vars[n].p.statpost_url = val;
	...
	val = (char *) ast_variable_retrieve(cfg,this,"prop_all_dtmf");
	if (val) rpt_vars[n].p.prop_all_dtmf = ast_true(val);
	val = (char *) ast_variable_retrieve(cfg,this,"dtmf_passthrough");
	if (val) rpt_vars[n].p.dtmf_passthrough = ast_true(val);
	...
	rpt_vars[n].p.extnodefilesn = 
	    explode_string(EXTNODEFILE,rpt_vars[n].p.extnodefiles,MAX_EXTNODEFILES,',',0);
	...
	if (val) rpt_vars[n].p.linkmode[LINKMODE_IRLP] = atoi(val);
        ...
        for(i = 0; i < nrpts; i++)
	{
                if(!strcmp(argv[2], rpt_vars[i].name))
		{
			...
			if ((strncasecmp(myrpt->rxchannel->name,"radio/", 6) == 0) &&
			    ...
			    (strncasecmp(myrpt->rxchannel->name,"rpiradio/", 9) == 0) &&
			    (strncasecmp(myrpt->rxchannel->name,"alsaradio/", 10) == 0)) return RESULT_SUCCESS;
			telem = myrpt->tele.next;
			while(telem != &myrpt->tele)
			{
...
//-------------------Somewhere in here, disable the other links------------------------------------//
	...
	/* if already in this mode, just ignore */
		...
		if ((!strncasecmp(l->chan->name,"echolink",8)) ||
			(!strncasecmp(l->chan->name,"tlb",3)) || (!strncasecmp(l->chan->name,"irlp",4)))
		{
			l->mode = mode;
	...
	if ((strncasecmp(s1,"iax2/", 5) == 0) || (strncasecmp(s1, "local/", 6) == 0) ||
	    (strncasecmp(s1,"echolink/",9) == 0) || (strncasecmp(s1,"tlb/",4) == 0) || (strncasecmp(s1,"irlp/",5) == 0))
#else
	if ((strncasecmp(s1,"iax2/", 5) == 0) || (strncasecmp(s1,"echolink/",9) == 0) ||
		(strncasecmp(s1,"tlb/",4) == 0) || (strncasecmp(s1,"irlp/",5) == 0))
#endif
	...
	*tele++ = 0;
	if ((!strncasecmp(deststr,"echolink",8)) || (!strncasecmp(deststr,"irlp",4)))
	{
		char tel1[100];
	...
	else if (node[0] == '9') init_linkmode(myrpt,l,LINKMODE_IRLP);

//---------------------------add code to not allow irlp if other links are on------------------------------------//
	...
	switch(myatoi(argv[0])){
		...
		case 56: /* RX CTCSS Enable */
			...
			if ((strncasecmp(myrpt->rxchannel->name,"radio/", 6) == 0) || 
			    (strncasecmp(myrpt->rxchannel->name,"simpleusb/", 10) == 0) ||
				(strncasecmp(myrpt->rxchannel->name,"rpiradio/", 9) == 0) ||
				(strncasecmp(myrpt->rxchannel->name,"alsaradio/", 10) == 0))
			{
		...
		case 62: /* same, without function complete (quietly, oooooooh baby!) */
			...
			if ((strncasecmp(myrpt->rxchannel->name,"radio/", 6) == 0) &&
			    ...
			    (strncasecmp(myrpt->rxchannel->name,"simpleusb/", 10) == 0) &&
			    (strncasecmp(myrpt->rxchannel->name,"rpiradio/", 9) == 0) &&
			    (strncasecmp(myrpt->rxchannel->name,"alsaradio/", 10) == 0)) break;
...
int	res;
	...
	else
	{
		...
		{
			if (mylink->lastrx && 
			    strncasecmp(mylink->chan->name,"echolink",8) && strncasecmp(mylink->chan->name,"irlp",4))
			{
				mylink->lastrealrx = 0;
				rpt_mutex_unlock(&myrpt->lock);
				return;
...
char	cmd[MAXDTMF+1] = "",c;
	...
	if ((myrpt->dtmfidx < 0) && myrpt->p.propagate_phonedtmf)
		do_dtmf_phone(myrpt,NULL,c);
//added here for repeating all dtmf
	if ((myrpt->dtmfidx < 0) && (myrpt->p.prop_all_dtmf))
	{		
		myrpt->mydtmf = c;
		do_dtmf_local(myrpt,c);	
	}	
	...
	while (ms >= 0)
	{
		...
		if (who == myrpt->monchannel) 
		{
			...
			if (f->frametype == AST_FRAME_VOICE)
			{
				...
				if (l->chan && (!strncasecmp(l->chan->name,"echolink",8)))
					fac = myrpt->p.etxgain;
				else if (l->chan && (!strncasecmp(l->chan->name,"irlp",4)))
					fac = myrpt->p.itxgain;
				if (fac != 1.0)
				{
...
//	if(altp>0)ast_log(LOG_NOTICE,"altp=%s \n",altp);
	...
	else
	{
		  ...
		  (strncasecmp(chan->name,"echolink",8)) && (strncasecmp(chan->name,"tlb",3)) && (strncasecmp(chan->name,"irlp",4)) ) {
	            ast_log(LOG_WARNING, "We only accept links via IAX2, Echolink, TheLinkBox, IRLP or Local!!\n");
	            return -1;
	        }
#else
		if (strncmp(chan->name,"IAX2",4) && strncasecmp(chan->name,"Echolink",8) && 
			strncasecmp(chan->name,"tlb",3) && strncasecmp(chan->name,"irlp",4))
		{
	...
	if (!myrpt->remote)
	{
		...
		if ((!phone_mode) && (l->name[0] != '0') &&
			...
			strncasecmp(chan->name,"tlb",3) && strncasecmp(chan->name,"irlp",4)) l->newkey = 2;
